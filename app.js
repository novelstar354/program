...
let starCanvasImages = {};

let starCanvasAnimations = {};

let starCanvasAnimationId = 0;

let starCanvasClickHandlers = [];
/* =====================================================
   STar CANVAS SYSTEM
===================================================== */
...
function starCanvasImage(
    ...
    if (!img) {
        ...
        log(
            `Canvas image "${name}" is not loaded.`
        );
...
function clearStarCanvasClicks() {
    ...
    for (
        const handler of
        starCanvasClickHandlers
    ) {

        starCanvas.removeEventListener(
            "click",
            handler
        );
...
function stopStarCanvasAnimation(
    ...
    if (
        ...
        delete starCanvasAnimations[name];
...
function stopAllStarCanvasAnimations() {

    for (
        const name in
        starCanvasAnimations
    ) {
...
if (
    ...
    if (!match) {
        ...
        runtimeError(
            'Canvas animate syntax: canvas animate "name" {',
            lineNumber,
            line
        );
...
if (
    ...
    if (!match) {
        ...
        runtimeError(
            'Canvas stop syntax: canvas stop "name"',
            lineNumber,
            line
        );
...
if (
    ...
    if (args.length < 5) {
        ...
        runtimeError(
            "Canvas arc requires x y radius start end",
            lineNumber,
            line
        );
...
if (
    ...
    if (args.length < 5) {
        ...
        runtimeError(
            "Canvas fillArc requires x y radius start end",
            lineNumber,
            line
        );
